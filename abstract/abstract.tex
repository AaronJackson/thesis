...
\begin{abstract}
  ...
  video games and trying on accessories online, to measuring emotion
  ...
  fail on cases of large poses, difficult expression, occlusion, and
  bad lighting. Additionally, encoding detail in such a small number
  of parameters is not possible.
  ...
  approach constrains the problem to the spatial domain using an
  end-to-end network which directly regresses the 3D geometry using a
  volumetric representation. This avoids the need for 3DMM generation,
  which involves finding correspondence between all vertices of all
  training samples, but also the fitting stage. We demonstrate that
  doing so can not only provide state-of-the-art results, but also be
  adapted to other deformable objects, such as the full human body.
\end{abstract}
