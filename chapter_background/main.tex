...
\begin{figure}
  ...
  \caption[The 2D Convolution]{The output of a 2D convolution
    operation on a $10 \times 10$ image with a $3 \times 3$ filter,
    using zero padding of one pixel on the input to result in an
...
input with a particular stride. We show a simple 2D convolution
operation using a $3 \times 3$ filter in
Figure~\ref{fig:convolution}. In this example, a one pixel padding has
been added to the input to allow the size of the output to match the
size of the input.
