...
shown in Fig.~\ref{fig:discretisation}. Notice that the process
...
volume size is shown in Fig.~\ref{fig:voxerror}. Given that the error
...
architecture is shown in Fig.~\ref{fig:cnnbaseline}. As it can be
...
Fig.~\ref{fig:cnnmultitask}, consisting of three hourglass
...
demonstrated in Fig.~\ref{fig:guidednet}. During training we used the
...
Figs.~\ref{roc:combined} and ~\ref{roc:florence}.  Visual results of
the proposed \textit{VRN - Guided} on some very challenging images
from AFLW2000-3D can be seen in Fig.~\ref{fig:aflw2000res}. Examples
of failure cases along with a visual comparison between \textit{VRN}
and \textit{VRN - Guided} can be found in the supplementary
material. From these results, we can conclude the following:
\begin{enumerate}
...
Fig.~\ref{fig:frontal_visual}. Numeric comparison is shown in
Fig.~\ref{roc:combined} (left), as \textit{VRN without alignment}. We
...
Fig.~\ref{fig:effect_pose}, the performance of our method decreases as
...
yet as shown in Fig.~\ref{fig:effect_expression}, the performance
